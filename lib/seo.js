...
export function postSeo({ post, site, renderedHtml }) {
  ...
  const author = [post.frontmatter?.author, configuredAuthor, siteName]
    ...
    .trim();
  const authorType = post.frontmatter?.authorType ?? 'Person';
  if (!['Organization', 'Person'].includes(authorType)) {
    throw new TypeError('authorType must be Organization or Person');
  }
  const authorUrl = post.frontmatter?.authorUrl == null
    ? null
    : absoluteHttpsUrl(post.frontmatter.authorUrl, 'authorUrl').toString();
  const authorProfile = authorType === 'Person' && author === configuredAuthor
    ? site.site?.authorProfile
    : null;
